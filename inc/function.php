...
//	cek_akses_langsung();
	if(empty($param)){
			...
			header('location:../index.php?mod=user&pg=register');
		exit();
...
function list_slider() {
	...
	while ($row = mysql_fetch_object($query)) {
			...
			echo "<li>
				<div class='seq-model'><img src='upload/banner/" . $row -> namaslider . "' /></div>
				</li>";
...
function paginationfront($halaman, $jumlah_halaman, $tabel, $page, $kategori) {
	if(!empty($kategori)){
		$baselink = "index.php?mod=" . $page . "&pg=" . $tabel . "&idkategori=" . $kategori ."&halaman=";
	}else{
		$baselink = "index.php?mod=" . $page . "&pg=" . $tabel . "&halaman=";
	}
	if($halaman > 1) {
		$previous = $halaman - 1;
		echo "<li><a href='" . $baselink . $previous . "' aria-label='Previous'><span aria-hidden='true'>&laquo;</span></i></a></li>";
	}
	...
	if($halaman < $jumlah_halaman) {
		$next = $halaman + 1;
		echo "<li><a href='" . $baselink . $next . "' aria-label='Next'><span aria-hidden='true'>&raquo;</span></a></li>";
	}
...
function combo_invoice($idpelanggan) {
	echo "<option value='' selected>- Pilih No Invoice -</option>";
	$query = query("SELECT noinvoice FROM invoice WHERE idpelanggan = '$idpelanggan'");
	while ($row = mysql_fetch_row($query)) {
